...
class Breakout:
    def __init__(self):
        # ____ Create root window ____
        self.root = tk.Tk()
        self.root.geometry("800x800") # add size
        self.root.resizable(False, False) # deny resizing

        self.change_direstion = None # Variable to trigle after_cancel function 
        self.blocks=[] # array to store block instances 

        # ____ Create blocks ____
        self.create_blocks()
        ...
        self.label_photo.place(x=350, y = 734, width=22, height=22)
    ...
    def create_blocks(self):
        # blocks_frame = tk.Frame(self.root) # Frame for blocks
        # blocks_frame.place(x=0, y=0, width=800)

        pad_y = 5 # First intend from top
        for i in range(5):
            ...
            pad_y += int(self.blocks[i].place_info()['height']) # new intend form top
    def direction_bullet(self):
        ...
        for block in self.blocks:
            ...
            bullet_x_start = int(self.label_photo.place_info()["x"])
            bullet_y_top = int(self.label_photo.place_info()['y'])
            
            bullet_x_center = bullet_x_start + (int(self.label_photo.place_info()['width'])/2)

            if block_x_start <= bullet_x_center and block_x_end >= bullet_x_center and bullet_y_top <= block_y_bottom and bullet_y_top >= block_y_top:
